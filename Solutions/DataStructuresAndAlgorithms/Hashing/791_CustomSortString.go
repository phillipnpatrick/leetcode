...
// at any position since it's not in order. The output "bcad" correctly follows this rule. Other arrangements like "dbca" or "bcda" would also 
...
func customSortString(order string, s string) string {
	...
	for j := 0; j < len(order); j++ {
		limit := frequency[order[j]] + index

		for index < limit {
			output[index] = order[j]
			index++
		}
		delete(frequency, order[j])
	}
